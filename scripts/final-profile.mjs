...
if (!s.includes("const PAYMENT_ADDRESS=import.meta.env.VITE_BNB_USDT_PAYMENT_ADDRESS||''")) {
  s = s.replace(
    'const EXAMPLES=',
    "const PAYMENT_ADDRESS=import.meta.env.VITE_BNB_USDT_PAYMENT_ADDRESS||'';\nconst EXAMPLES="
  );
}
...
if (!s.includes("const[txHash,setTxHash]=useState('')")) {
  ...
  if (s.includes(marker)) {
    s = s.replace(
      marker,
      marker + "const[txHash,setTxHash]=useState('');const[verifyState,setVerifyState]=useState('');"
    );
  }
}
...
if (i >= 0) {
  ...
  for (let p = i; p <= s.length - 10; p++) {
    if (s.startsWith('<section', p)) {
      depth++;
    } else if (s.startsWith('</section>', p)) {
      depth--;
      if (depth === 0) {
