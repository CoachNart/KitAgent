...
if(!s.includes("./integrations/opensea.js"))s=s.replace("import {classifyTerminalCommand,parseTerminalNft,looksLikeAddress} from './agent/terminal-agent.js';","import {classifyTerminalCommand,parseTerminalNft,looksLikeAddress} from './agent/terminal-agent.js';\nimport {getOwnedNFTs,getNFT} from './integrations/opensea.js';");
...
if(i>=0&&j>i){const fn=` async function buildPlan(){
  ...
  try{
    ...
    if(/\\b(show|list|view)\\b.*\\b(my )?(nfts?|collectibles)\\b/i.test(text)){
      if(!account)throw new Error('Connect your wallet first so I can discover your NFTs.');
      setAction('nft');const d=await getOwnedNFTs(account,chainKey);const items=d.nfts||[];if(!items.length){setNotice('I could not find NFTs for this wallet on the selected network.');}else{const names=items.slice(0,8).map((x,i)=>\`\${i+1}. \${x.name||x.collection||'Unnamed NFT'} #\${x.identifier??x.token_id??'?'}\`).join(' • ');setNotice(\`Found \${items.length} NFT(s): \${names}\`)}setCommand('');return;
    }
    if(/\\b(sell|list)\\b.*\\bnft\\b/i.test(text)){setAction('nft');setNotice('Tell me the NFT contract/token ID and asking price. I will retrieve the OpenSea listing actions and keep wallet signing as the final step.');setCommand('');return}
    ...
    if(agentKind==='swap'){setAction('swap');const q=parseSwap(text);if(!q.amount||!q.from||!q.to){setNotice('I need the amount and both tokens. Example: “swap 25 USDC for ETH”.');setCommand('');return}await findRoute(false);setNotice('Swap route prepared. Review the quote before wallet approval.');setCommand('');return}
