...
export default function NotificationCenter({user,embedded=false}){
 ...
 return <div className={embedded?'notification-center embedded':'notification-center'} style={embedded?wrapEmbedded:wrapFloating}>
  ...
  {open&&<div className="notification-panel" style={embedded?panelEmbedded:panel}>
   <div className="notification-head" style={head}><div><strong>Live activity</strong><div style={sub}>Real-time structure, alerts & scheduled news</div></div><button onClick={()=>setOpen(false)} style={close}><X size={16}/></button></div>
   <div className="notification-status" style={statusCard}><div style={statusTop}><span><Radio size={12}/> BYBIT LIVE</span><small>{market.checkedAt?`Checked ${fmtTime(market.checkedAt)}`:'Starting monitor…'}</small></div><div className="notification-status-grid" style={statusGrid}><div><b>{liveMarkets.filter(x=>x.ok).length}/{SYMBOLS.length}</b><span>symbols online</span></div><div><b>5m</b><span>structure</span></div><div><b>{upcoming.length}</b><span>high impact</span></div></div>{market.error&&<div style={error}>{market.error}</div>}</div>
   {liveEvent&&<div className="notification-event" style={eventCard}><div style={eventIcon}><Zap size={13}/></div><div><b>{liveEvent.symbol?.replace('USDT','')} {liveEvent.direction} {liveEvent.type||'BOS'}</b><span>{fmtPrice(liveEvent.level)} · {liveEvent.timeframe||'live'} · detected {fmtTime(liveEvent.detectedAt)}</span></div></div>}
   {activity.length>0&&<div className="notification-section" style={section}><div className="notification-section-title" style={sectionTitle}>RECENT ACTIVITY</div>{activity.slice(0,4).map(a=><div style={newsItem} key={a.key}><div><b>{a.symbol?.replace('USDT','')} {a.direction} {a.type}</b><span>{a.timeframe} · {fmtTime(a.detectedAt)}</span></div></div>)}</div>}
   ...
   {permission!=='granted'&&<button onClick={enable} className="notification-enable" style={enableBtn}><Bell size={14}/> Enable browser notifications</button>}
