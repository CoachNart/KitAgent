...
export default function CommandAgentBridge({ children }) {
  ...
  useEffect(()=>{
    const find=()=>document.querySelector('.command-page .command-card');
    const observer=new MutationObserver(()=>{const el=find();if(el)setTarget(el);});
    observer.observe(document.body,{childList:true,subtree:true});
    const el=find();if(el)setTarget(el);
  ...
  const run=useCallback(async raw=>{
    ...
    }catch(error){setEntries(e=>[...e,{id:`e-${Date.now()}`,role:'agent',text:error?.message||'Execution planning failed.'}]);}
  ...
  const portal=useMemo(()=>{if(!target)return null;return createPortal(<div className="bridge-conversation conversation">
    {entries.map(entry=><div key={entry.id} className={entry.role==='user'?'message user-message':'message agent-message'}><span className="message-mark">{entry.role==='user'?null:<Terminal size={13}/>}</span><div><small>{entry.role==='user'?'YOU':'KITAGENT'}</small><p>{entry.text}</p>
      {entry.meta?.execution&&<div className="agent-execution-card"><div className="agent-execution-head"><span><Terminal size={13}/> LIVE EXECUTION PLAN</span><b>{String(entry.meta.intent||'ACTION').toUpperCase()}</b></div><div className="agent-execution-grid">
        <div><small>ADAPTER</small><b>{entry.meta.execution.adapter||'agent'}</b></div><div><small>STEPS</small><b>{entry.meta.execution.transactions?.length||1}</b></div><div><small>CHAIN</small><b>{entry.meta.execution.chainId||4663}</b></div><div><small>STATE</small><b>{entry.meta.execution.state||'prepared'}</b></div>
      </div><div className="agent-execution-warning"><ShieldAlert size={14}/><span>Simulation completed where safe. Nothing is signed until you approve.</span></div></div>}
