...
export default function AgentTerminalLayer(){
 ...
 const [logs,setLogs]=useState([{kind:'system',text:'KitSetups execution terminal ready.'},{kind:'system',text:'Awaiting a task. Live data tools are connected.'}]);
 const [result,setResult]=useState(null);
 const [visible,setVisible]=useState(true);
 useEffect(()=>{const root=document.querySelector('.kit-shell');if(!root)return;const sync=()=>{const selected=root.querySelector('.side-link.selected');const active=selected?.textContent?.toLowerCase().includes('command center')??true;setVisible(active);document.body.classList.toggle('agent-terminal-active',active)};sync();const mo=new MutationObserver(sync);mo.observe(root,{subtree:true,attributes:true,attributeFilter:['class']});return()=>{mo.disconnect();document.body.classList.remove('agent-terminal-active')}},[]);
 ...
 return <section className="agent-terminal-shell">
   ...
   <div className="agent-terminal-body">
    <div className="agent-output">
      ...
      {best&&<div className="opportunity-output"><div className="opp-kicker">BEST CURRENT OPPORTUNITY</div><div className="opp-name">{best.name||best.symbol||best.marketId}</div><div className="opp-grid"><div><span>TYPE</span><b>{best.type}</b></div><div><span>APY</span><b>{fmtPct(best.avgNetApy??best.avgNetApyExcludingRewards??best.supplyApy)}</b></div><div><span>TVL</span><b>{fmtUsd(best.totalAssetsUsd??best.tvl)}</b></div><div><span>LIQUIDITY</span><b>{fmtUsd(best.liquidityUsd??best.liquidity)}</b></div></div><div className="opp-note">Ranked from live Morpho data. APY, TVL and liquidity are shown only when returned by the protocol.</div></div>}</div>
    </div>
    <div className="agent-input-wrap"><div className="input-label"><span><Command size={13}/> TASK INPUT</span><span>Ctrl/⌘ + Enter</span></div><div className="agent-input"><Command size={17}/><textarea value={command} onChange={e=>setCommand(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&(e.ctrlKey||e.metaKey)){e.preventDefault();submit()}}} placeholder="Give KitSetups a task…"/><button onClick={()=>submit()} disabled={running||!command.trim()}><Play size={14}/>{running?'Running':'Run task'}</button></div><button className="terminal-suggestion" onClick={()=>{setCommand(prompt);submit(prompt)}} disabled={running}>Run: {prompt}</button></div>
   </div>
