...
export default function LiveMarketPage(){const [market,setMarket]=useState('forex'),[pair,setPair]=useState(''),[timeframe,setTimeframe]=useState('1H'),[strategy,setStrategy]=useState('TOP_DOWN'),[loading,setLoading]=useState(false),[sourceLoading,setSourceLoading]=useState(true),[error,setError]=useState(''),[result,setResult]=useState(null),[savedSignal,setSavedSignal]=useState(null),[instrumentQuery,setInstrumentQuery]=useState(''),[instruments,setInstruments]=useState([]),[pickerOpen,setPickerOpen]=useState(false);
 ...
 const localInstruments=(m)=>m==='perpetual'?CRYPTO.map(symbol=>({symbol})):[];
 ...
 useEffect(()=>{let cancelled=false;setError('');setInstrumentQuery('');setPickerOpen(false);setSourceLoading(true);setResult(null);const local=localInstruments(market);setInstruments(local);setPair(local[0]?.symbol||'');(async()=>{try{const token=auth?.currentUser?await auth.currentUser.getIdToken():'';const r=await fetch('/api/market?action=instruments&market='+encodeURIComponent(market),{headers:token?{Authorization:'Bearer '+token}:{},cache:'no-store'});const body=await r.json().catch(()=>({}));if(cancelled)return;if(!r.ok||!Array.isArray(body?.instruments)||!body.instruments.length){setInstruments([]);setPair('');setError(market==='forex'?'No Biquote Forex instruments are currently available. Please retry.':'No Biquote Metal / CFD instruments are currently available. Please retry.');return}const live=body.instruments.map(x=>({symbol:x.symbol,name:x.name||''}));setInstruments(live);setPair(p=>live.some(x=>x.symbol===p)?p:(live[0]?.symbol||''));}catch{if(!cancelled){setInstruments([]);setPair('');setError('Unable to load live instruments from the market-data source. Please retry.')}}finally{if(!cancelled)setSourceLoading(false)}})();return()=>{cancelled=true}},[market]);
