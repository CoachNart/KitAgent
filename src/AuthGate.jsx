...
import { getDeviceBindingId, getDeviceBindingFingerprint } from './deviceBinding.js';
...
async function registerDevice(user){const deviceId=await getDeviceBindingId();const deviceFingerprint=await getDeviceBindingFingerprint();const response=await authorizedRequest(user,'/api/register-device',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({deviceId,deviceFingerprint})});let payload={};try{payload=await response.json()}catch{}if(!response.ok)throw Object.assign(new Error(payload.error||'This device cannot be registered.'),{code:payload.code||'DEVICE_REGISTRATION_FAILED'});return deviceId}
