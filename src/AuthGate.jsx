...
import { browserLocalPersistence, browserSessionPersistence, createUserWithEmailAndPassword, getRedirectResult, GoogleAuthProvider, onAuthStateChanged, setPersistence, signInWithEmailAndPassword, signInWithPopup, signInWithRedirect } from 'firebase/auth';
...
async function ensureAuthPersistence(){
  ...
  try{await setPersistence(auth,browserLocalPersistence)}catch(error){
    console.warn('KitAgent local auth persistence unavailable; using session persistence.',error);
    try{await setPersistence(auth,browserSessionPersistence)}catch(fallbackError){
      console.warn('KitAgent session auth persistence unavailable; continuing with in-memory auth.',fallbackError);
    }
  }
}
...
async function registerDevice(user){
  ...
  const response=await fetch('/api/register-device',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({deviceId})});
  let payload={}; try{payload=await response.json()}catch{}
  if(!response.ok) throw new Error(payload.error||'This device cannot be registered.');
  return deviceId;
}
