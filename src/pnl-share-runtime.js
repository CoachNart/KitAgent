/* Browser-safe PnL image actions. Scoped to KitSetups Futures position rows only. */
(function installPnlShareRuntime(){
  ...
  function parsePosition(text){
    ...
    const side = (raw.match(/(?:[A-Z0-9_/-]+\s*[·•]\s*)(Long|Short)/i)?.[1] || 'Long').toLowerCase();
    const symbol = raw.match(/([A-Z0-9_/-]+)\s*[·•]\s*(?:Long|Short)/i)?.[1] || 'BTC/USDT';
  ...
  function makePngFile(position){
    return new Promise((resolve,reject)=>{
      try{
        ...
        canvas.toBlob(blob=>{if(!blob)return reject(new Error('PNG conversion failed'));resolve(new File([blob],'kitsetups-futures-pnl.png',{type:'image/png'}));},'image/png');
  ...
  async function runAction(text,share){
    ...
    if(!p.entry || !p.mark) throw new Error('Could not read Entry and Mark from the position row');
  ...
  function findPositionRow(target){
    const row=target?.closest?.('tr');
    if(!row) return null;
    const text=String(row.textContent||'');
    return /Entry\s+[0-9,]+/i.test(text) && /Mark\s+[0-9,]+/i.test(text) && /Leverage\s+[0-9.]+x/i.test(text) ? row : null;
  }
  ...
  async function handleClick(event){
    ...
    const row=findPositionRow(event.target); if(!row)return;
    event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation();
    try{await runAction(row.textContent,action==='share')}catch(error){console.error('KitSetups PnL image action failed:',error)}
  }
