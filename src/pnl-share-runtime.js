...
(function installPnlShareRuntime(){
  ...
  function drawAvatar(ctx,avatar,x,y,size,profile,accent){ctx.save();ctx.beginPath();ctx.arc(x+size/2,y+size/2,size/2,0,Math.PI*2);ctx.clip();if(avatar?.tagName==='IMG'&&avatar.complete&&avatar.naturalWidth){try{ctx.drawImage(avatar,x,y,size,size)}catch{ctx.fillStyle='#15212b';ctx.fillRect(x,y,size,size)}}else{ctx.fillStyle='#15212b';ctx.fillRect(x,y,size,size);ctx.fillStyle=accent;ctx.font='800 52px Arial,sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(profile.slice(0,1).toUpperCase(),x+size/2,y+size/2)}ctx.restore();ctx.strokeStyle='rgba(37,214,208,.55)';ctx.lineWidth=4;ctx.beginPath();ctx.arc(x+size/2,y+size/2,size/2,0,Math.PI*2);ctx.stroke();}
