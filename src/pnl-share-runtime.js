/* Browser-safe PnL image actions. Scoped to the KitSetups Futures PnL card only. */
(function installPnlShareRuntime(){
  ...
  function parsePosition(text){
    ...
    const entry = n((raw.match(/Entry\s+([0-9,]+(?:\.[0-9]+)?)/i)?.[1] || '0').replace(/,/g,''));
    const mark = n((raw.match(/Mark\s+([0-9,]+(?:\.[0-9]+)?)/i)?.[1] || '0').replace(/,/g,''));
    const side = (raw.match(/(?:Futures\s+[A-Z0-9_/-]+\s*[·•]\s*|[A-Z0-9_/-]+\s*[·•]\s*)(Long|Short)/i)?.[1] || 'Long').toLowerCase();
    const symbol = raw.match(/Futures\s+([A-Z0-9_/-]+)\s*[·•]\s*(?:Long|Short)/i)?.[1] || raw.match(/^\s*([A-Z0-9_/-]+)\s*[·•]\s*(?:long|short)/im)?.[1] || 'BTC/USDT';
    const name = (raw.match(/^\s*([^\n]+?)\s*[·•]\s*KitSetups Futures/i)?.[1] || 'KitSetups Trader').trim();
    const leverage = raw.match(/Leverage\s+([0-9.]+)x/i)?.[1] || '—';
  ...
  function makePngFile(position){
    return new Promise((resolve,reject)=>{
      try{
        ...
        text(p.name,88,310,38,'#eef3f9','800');
        text(p.symbol,88,355,24,'#738398','400');
        ...
        text(fmt(p.entry,8),88,832,30,'#eef3f9','700');
        text('MARK',540,790,18,'#66768a','700');
        text(fmt(p.mark,8),540,832,30,'#eef3f9','700');
        text('LEVERAGE',88,920,18,'#66768a','700');
        text(`${p.leverage}x`,88,962,30,'#eef3f9','700');
        text(`KitSetups Futures · ${p.symbol} · ${p.side}`,88,1780,18,'#66768a','400');
        text('kitsetups.xyz',88,1820,18,accent,'800');

        canvas.toBlob(blob=>{
          if(!blob) return reject(new Error('PNG conversion failed'));
          resolve(new File([blob],'kitsetups-futures.png',{type:'image/png'}));
        },'image/png');
  ...
  async function runAction(text, share){
    const p=parsePosition(text);
    if(!p.entry || !p.mark) throw new Error('Could not read Entry and Mark from the PnL card');
    const file=await makePngFile(p);
  ...
  function findPnlCard(target){
    let node=target;
    while(node && node!==document.body){
      const text=String(node.textContent||'');
      if(/Entry\s+[0-9,]+/i.test(text) && /Mark\s+[0-9,]+/i.test(text) && /Leverage\s+[0-9.]+x/i.test(text) && /KitSetups/i.test(text)) return node;
  ...
  async function handleClick(event){
    const action=isAction(event.target);
    if(!action) return;
    const card=findPnlCard(event.target);
    if(!card) return;
    event.preventDefault();
    event.stopPropagation();
    event.stopImmediatePropagation();
    try{ await runAction(card.textContent,action==='share'); }
    catch(error){ console.error('KitSetups PnL image action failed:',error); }
  }

  document.addEventListener('click',handleClick,true);
  new MutationObserver(()=>{}).observe(document.documentElement,{subtree:true,childList:true});
})();
