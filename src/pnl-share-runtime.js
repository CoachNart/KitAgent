...
(function installPnlShareRuntime(){
  ...
  function shareSvg(text){
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1920" viewBox="0 0 1080 1920"><defs><linearGradient id="bg" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#060a10"/><stop offset=".55" stop-color="#0b151e"/><stop offset="1" stop-color="#071015"/></linearGradient></defs><rect width="1080" height="1920" rx="58" fill="url(#bg)"/><rect x="48" y="48" width="984" height="1824" rx="46" fill="none" stroke="#263141" stroke-width="2"/><text x="88" y="126" fill="${accent}" font-family="Arial,sans-serif" font-size="30" font-weight="800">KITSETUPS</text><text x="88" y="166" fill="#66768a" font-family="Arial,sans-serif" font-size="18">FUTURES POSITION · ${escapeXml(side.toUpperCase())}</text><text x="88" y="310" fill="#eef3f9" font-family="Arial,sans-serif" font-size="38" font-weight="800">${escapeXml(name)}</text><text x="88" y="355" fill="#738398" font-family="Arial,sans-serif" font-size="24">${escapeXml(symbol)}</text><text x="88" y="480" fill="#66768a" font-family="Arial,sans-serif" font-size="20">POSITION PERFORMANCE</text><text x="88" y="600" fill="${accent}" font-family="Arial,sans-serif" font-size="96" font-weight="900">${percentage >= 0 ? '+' : ''}${fmt(percentage,4)}%</text><rect x="88" y="710" width="904" height="2" fill="#202c38"/><text x="88" y="790" fill="#66768a" font-family="Arial,sans-serif" font-size="18">ENTRY</text><text x="88" y="832" fill="#eef3f9" font-family="Arial,sans-serif" font-size="30" font-weight="700">${escapeXml(entry)}</text><text x="540" y="790" fill="#66768a" font-family="Arial,sans-serif" font-size="18">MARK</text><text x="540" y="832" fill="#eef3f9" font-family="Arial,sans-serif" font-size="30" font-weight="700">${escapeXml(mark)}</text><text x="88" y="920" fill="#66768a" font-family="Arial,sans-serif" font-size="18">LEVERAGE</text><text x="88" y="962" fill="#eef3f9" font-family="Arial,sans-serif" font-size="30" font-weight="700">${escapeXml(leverage)}x</text><rect x="88" y="1080" width="904" height="260" rx="30" fill="#0b121a" stroke="#202c38"/><text x="124" y="1140" fill="${accent}" font-family="Arial,sans-serif" font-size="18" font-weight="800">KITSETUPS · FUTURES SNAPSHOT</text><text x="124" y="1200" fill="#aab6c7" font-family="Arial,sans-serif" font-size="22">Price performance from entry to current mark</text><text x="124" y="1255" fill="#66768a" font-family="Arial,sans-serif" font-size="18">No margin or USDT PnL included</text><text x="88" y="1780" fill="#66768a" font-family="Arial,sans-serif" font-size="18">${escapeXml(new Date().toLocaleString())}</text><text x="88" y="1820" fill="${accent}" font-family="Arial,sans-serif" font-size="18" font-weight="800">kitsetups.xyz</text></svg>`;
  }
    ...
    navigator.share = async data => {
      if(data?.title === 'KitSetups Futures PnL' && data?.text){
        ...
        try {
          ...
          // Do not use canShare() as a gate: some Android browsers report false
          // even though the native share sheet accepts image files.
          try { return await nativeShare({title:'KitSetups Futures',files:[file]}); }
          catch(_) { downloadFile(file); return; }
        } catch(_) {
          // If image generation fails, keep the share payload percentage-only.
  ...
  document.addEventListener('click', async event => {
    ...
    try {
      ...
      const file = await svgToPngFile(shareSvg(cleanShareText(svg)),'kitsetups-futures.png');
      if(navigator.share){
        try { await navigator.share({title:'KitSetups Futures',files:[file]}); }
        catch(_) { downloadFile(file); }
