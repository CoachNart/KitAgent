...
export default function HistoryPage() {
 ...
 const exportCsv=()=>{const body=['Date,Type,Asset,Amount,USD,Gas,Network,Status,Hash,P&L',...rows.map(t=>[t.date,t.type,t.asset,t.amount,t.usd,t.gas,t.network,t.status,t.hash,t.pnl].map(v=>`"${String(v).replaceAll('"','""')}"`).join(','))].join('\n'); const blob=new Blob([body],{type:'text/csv'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='kitagent-transactions.csv'; a.click(); URL.revokeObjectURL(a.href); };
