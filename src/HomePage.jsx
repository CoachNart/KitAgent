...
function CountUp({value,duration=900}){const target=Number(value)||0;const [shown,setShown]=useState(0);const [active,setActive]=useState(false);useEffect(()=>{let raf=0,start=null;setShown(0);setActive(true);const tick=t=>{if(start===null)start=t;const p=Math.min(1,(t-start)/duration);const eased=1-Math.pow(1-p,3);setShown(Math.round(target*eased));if(p<1)raf=requestAnimationFrame(tick);else{setShown(target);setActive(false)}};raf=requestAnimationFrame(tick);return()=>cancelAnimationFrame(raf)},[target,duration]);return <b className={'snapshot-number '+(active?'is-counting':'')} aria-label={String(target)}>{shown.toLocaleString()}</b>}
