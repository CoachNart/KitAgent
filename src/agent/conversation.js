...
export function parseTokenContract(text=''){const a=addresses(text);return a[0]||null}
export function parseSpender(text=''){const a=addresses(text);return a[1]||null}
...
export function parseNft(text=''){
  const a=addresses(text);
  const id=String(text).match(/(?:nft|token|#|id)\s*#?\s*(\d+)/i)?.[1]||String(text).match(/#(\d+)/)?.[1]||null;
  return {addresses:a,recipient:a[0]||null,contract:a[1]||null,tokenId:id}
}
export function parseSwap(text=''){
  const amount=parseAmount(text); const symbols=[...String(text).matchAll(/\b(USDC|USDT|DAI|WETH|ETH|BNB|MATIC)\b/gi)].map(m=>m[1].toUpperCase());
  return {amount,from:symbols[0]||null,to:symbols[1]||null}
}
export function parseBridge(text=''){
  const p=parseSwap(text); const m=String(text).match(/\bto\s+([A-Za-z0-9 _-]+?)(?:\s|$)/i); return {...p,destination:m?.[1]?.trim()||null}
}
export function parseDeFiCommand(text=''){
  const l=String(text).toLowerCase(); const action=l.match(/\b(supply|deposit|borrow|repay|withdraw|stake|unstake)\b/)?.[1]||null; return {action,amount:parseAmount(text),token:parseTokenSymbol(text)}
}
export function parseApproval(text=''){
  const a=addresses(text); return {token:a[0]||null,spender:a[1]||null,amount:parseAmount(text),symbol:parseTokenSymbol(text)}
}
