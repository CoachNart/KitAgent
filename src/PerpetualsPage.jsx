...
async function api(action, state, extra = {}) {
  ...
  if (!response.ok || data?.error) throw new Error(data?.error || `MEXC request failed (${response.status})`);
  return data;
}
...
export default function PerpetualsPage() {
  const [symbol, setSymbol] = useState(() => localStorage.getItem('kit_mexc_symbol') || 'BTCUSDT');
  ...
  const loadAccount = useCallback(async () => {
    ...
    if (firstError) setError(firstError.reason?.message || 'One MEXC account feed failed.');
  }, [connected, key, secret, state]);

  useEffect(() => { loadMarket(true); }, []);
  useEffect(() => { if (!pairs.length) return; localStorage.setItem('kit_mexc_symbol', symbol); loadMarket(false); }, [symbol, interval]);
  ...
  const connect = async e => { e?.preventDefault(); if (!key || !secret) { setError('Enter your MEXC Futures Access Key and Secret Key.'); return; } setBusy(true); setError(''); try { await api('connect', state); setConnected(true); setCredentialsOpen(false); } catch (e) { setConnected(false); setError(e.message || 'MEXC connection failed.'); } finally { setBusy(false); } };
  ...
  const placeOrder = async e => {
    ...
    const price = orderType === 'market' ? last : n(limitPrice); if (!price) { setError('Enter a valid limit price.'); return; }
    setBusy(true); setError('');
    try { await api('order', state, { side, intent: reduceOnly ? 'close' : 'open', type: orderType === 'market' ? 5 : 1, marginMode, leverage, volume: vol, price, reduceOnly, takeProfit: n(takeProfit) || undefined, stopLoss: n(stopLoss) || undefined }); setVolume(''); setTakeProfit(''); setStopLoss(''); await loadAccount(); }
    catch (e) { setError(e.message || 'MEXC rejected the order.'); } finally { setBusy(false); }
  };
  ...
  const estimatedMargin = n(volume) && last ? (n(volume) * last * n(contract?.contractSize || 1)) / Math.max(1, n(leverage)) : 0;
  ...
  return <div className="mexc-terminal">
    <header className="mexc-topbar">
      <div className="mexc-brand"><span className="mexc-logo">M</span><div><strong>MEXC Futures</strong><small>USDT-M Perpetual</small></div></div>
      ...
      <div className="mexc-connection"><span className={ticker ? 'dot live' : 'dot'} /> {connected ? 'Account connected' : 'Market live'} <button onClick={() => connected ? disconnect() : setCredentialsOpen(true)}>{connected ? 'Disconnect' : 'Connect MEXC'}</button></div>
    </header>
    ...
    <main className="mexc-main">
      ...
      <section className="mexc-order-panel"><div className="panel-title"><b>Place Order</b><span>Perpetual</span></div><div className="order-sides"><button className={side === 'buy' ? 'active buy' : ''} onClick={() => setSide('buy')}>Open Long</button><button className={side === 'sell' ? 'active sell' : ''} onClick={() => setSide('sell')}>Open Short</button></div><div className="field-row"><label>Margin</label><select value={marginMode} onChange={e => setMarginMode(e.target.value)}><option value="cross">Cross</option><option value="isolated">Isolated</option></select></div><div className="field"><label>Leverage <b>{leverage}x</b></label><input type="range" min="1" max={maxLeverage} value={leverage} onChange={e => setLeverage(Number(e.target.value))}/><div className="range-labels"><span>1x</span><span>{maxLeverage}x</span></div></div><div className="field-row"><label>Order Type</label><select value={orderType} onChange={e => setOrderType(e.target.value)}><option value="market">Market</option><option value="limit">Limit</option></select></div>{orderType === 'limit' && <Field label="Price" value={limitPrice} onChange={setLimitPrice} placeholder={fmt(last)} />}<Field label="Size (contracts)" value={volume} onChange={setVolume} placeholder="Enter contract quantity" /><div className="quick-size">{[25,50,75,100].map(v => <button key={v} type="button" onClick={() => setVolume(String(Math.max(1, Math.floor(n(usdt.availableBalance) * v / 100 * n(leverage) / Math.max(last * n(contract?.contractSize || 1), 1)))))}>{v}%</button>)}</div><div className="estimate"><span>Est. initial margin</span><b>{estimatedMargin ? `${fmt(estimatedMargin, 4)} USDT` : '—'}</b></div><Field label="Take Profit" value={takeProfit} onChange={setTakeProfit} placeholder="Optional" /><Field label="Stop Loss" value={stopLoss} onChange={setStopLoss} placeholder="Optional" /><label className="check"><input type="checkbox" checked={reduceOnly} onChange={e => setReduceOnly(e.target.checked)} /> Reduce-only</label><button className={side === 'buy' ? 'submit buy-submit' : 'submit sell-submit'} onClick={placeOrder} disabled={busy}>{busy ? 'Processing…' : connected ? `${side === 'buy' ? 'Open Long' : 'Open Short'} ${displaySymbol(symbol)}` : 'Connect MEXC to Trade'}</button><div className="available">Available <b>{fmt(usdt.availableBalance, 4)} USDT</b></div></section>
    </main>
...
function CandleChart({ data }) { const w = 1000, h = 460, pad = 30, max = Math.max(...data.map(x => x.high), 0), min = Math.min(...data.map(x => x.low), max || 1), range = max - min || 1, visible = data.slice(-120); return <div className="candle-wrap"><svg viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" className="candle-chart"><rect width="100%" height="100%" fill="#080c12"/>{[1,2,3,4].map(i => <line key={i} x1="0" x2={w} y1={(h / 5) * i} y2={(h / 5) * i} stroke="#18212d" />)}{visible.map((c, i) => { const x = pad + i * ((w - pad * 2) / Math.max(1, visible.length - 1)); const y = v => pad + ((max - v) / range) * (h - pad * 2); const up = c.close >= c.open; return <g key={c.time || i}><line x1={x} x2={x} y1={y(c.high)} y2={y(c.low)} stroke={up ? '#22c7a5' : '#f05b6b'} /><rect x={x - 2} y={Math.min(y(c.open), y(c.close))} width="4" height={Math.max(2, Math.abs(y(c.open) - y(c.close)))} fill={up ? '#22c7a5' : '#f05b6b'} /></g>; })}</svg>{!visible.length && <div className="chart-empty">Loading MEXC candles…</div>}</div>; }
function Empty({ text }) { return <div className="table-empty">{text}</div>; }
function Positions({ rows, onClose }) { if (!rows.length) return <Empty text="No open positions for this contract." />; return <table><thead><tr><th>Contract</th><th>Side</th><th>Size</th><th>Entry</th><th>Liquidation</th><th>Margin</th><th>Leverage</th><th>Margin Ratio</th><th>Unrealized PnL</th><th>Action</th></tr></thead><tbody>{rows.map(p => <tr key={p.positionId}><td>{displaySymbol(p.symbol)}</td><td className={n(p.positionType) === 1 ? 'bid' : 'ask'}>{n(p.positionType) === 1 ? 'Long' : 'Short'}</td><td>{fmt(p.holdVol)}</td><td>{fmt(p.holdAvgPrice)}</td><td>{fmt(p.liquidatePrice)}</td><td>{fmt(p.im)}</td><td>{fmt(p.leverage, 0)}x</td><td>{pct(p.marginRatio)}</td><td className={n(p.unRealizedPnl) >= 0 ? 'bid' : 'ask'}>{fmt(p.unRealizedPnl)}</td><td><button className="row-action" onClick={() => onClose(p)}>Close</button></td></tr>)}</tbody></table>; }
...
function Risk({ risk, fee, positionMode, contract }) { const entries = risk ? Object.entries(risk).flatMap(([symbol, levels]) => Array.isArray(levels) ? levels.map(x => ({ symbol, ...x })) : []) : []; return <div className="risk-grid"><div className="risk-card"><small>Position mode</small><b>{n(positionMode?.positionMode) === 1 ? 'Hedge Mode' : n(positionMode?.positionMode) === 2 ? 'One-way Mode' : '—'}</b></div><div className="risk-card"><small>Contract size</small><b>{fmt(contract?.contractSize)}</b></div><div className="risk-card"><small>Volume unit</small><b>{fmt(contract?.volUnit)}</b></div><div className="risk-card"><small>Price unit</small><b>{fmt(contract?.priceUnit)}</b></div>{entries.slice(0, 12).map((r, i) => <div className="risk-card" key={i}><small>Risk L{r.level} · {n(r.positionType) === 1 ? 'Long' : 'Short'}</small><b>MMR {pct(r.mmr)} · IMR {pct(r.imr)} · Max {fmt(r.maxLeverage, 0)}x</b></div>)}<div className="risk-card"><small>Fee source</small><b>{fee ? 'MEXC account fee data' : 'MEXC Futures API'}</b></div></div>; }
