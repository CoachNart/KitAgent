...
export default function PerpetualsPage({ user }) {
  ...
  const API_TAKER_FEE_RATE = 0.0008; // MEXC API Futures taker fee effective Jun 1, 2026
  const estimatedNotional = n(volume) * orderEntry * orderContractSize;
  const estimatedMargin = estimatedNotional ? estimatedNotional / Math.max(1, n(leverage)) : 0;
  const estimatedFee = estimatedNotional * API_TAKER_FEE_RATE;
  const estimatedTotalCost = estimatedMargin + estimatedFee;
  const riskMmr = (() => {
  ...
  return <div className="mexc-terminal">
    ...
    <main className="mexc-main">
      <section className="mexc-chart-panel" ref={chartRef}><div className="mexc-toolbar"><div className="mexc-timeframes">{['1m','5m','15m','30m','1h','4h','1d'].map(tf => <button key={tf} className={interval === tf ? 'active' : ''} onClick={() => setIntervalValue(tf)}>{tf}</button>)}</div><span>● Live</span></div><CandleChart data={candleData} /></section>
