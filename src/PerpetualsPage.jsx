  ...
  const sharePnl = async p => {
    ...
    try {
      if (typeof navigator.share === 'function') {
        try {
          if (navigator.canShare?.({ files: [file] })) { await navigator.share({ title: 'KitSetups Futures PnL', text, files: [file] }); return; }
        } catch (e) { if (e?.name === 'AbortError') return; }
        try { await navigator.share({ title: 'KitSetups Futures PnL', text }); return; }
        catch (e) { if (e?.name === 'AbortError') return; }
      }
      triggerPnlDownload(file);
    } catch (e) { setError(e?.message || 'Could not share the PnL card.'); }
  };

  const downloadPnl = () => {
    if (pnlShareFile) triggerPnlDownload(pnlShareFile);
    else setError('PNL card is still preparing. Please tap Download again.');
  };
...
function Risk({ risk, fee, positionMode, contract }) { const entries = risk ? Object.entries(risk).flatMap(([symbol, levels]) => Array.isArray(levels) ? levels.map(x => ({ symbol, ...x })) : []) : []; return <div className="risk-grid"><div className="risk-card"><small>Position mode</small><b>{n(positionMode?.positionMode) === 1 ? 'Hedge Mode' : n(positionMode?.positionMode) === 2 ? 'One-way Mode' : '—'}</b></div><div className="risk-card"><small>Contract size</small><b>{fmt(contract?.contractSize)}</b></div><div className="risk-card"><small>Volume unit</small><b>{fmt(contract?.volUnit)}</b></div><div className="risk-card"><small>Price unit</small><b>{fmt(contract?.priceUnit)}</b></div>{entries.slice(0, 12).map((r, i) => <div className="risk-card" key={i}><small>Risk L{r.level} · {n(r.positionType) === 1 ? 'Long' : 'Short'}</small><b>MMR {pct(r.mmr)} · IMR {pct(r.imr)} · Max {fmt(r.maxLeverage, 0)}x</b></div>)}<div className="risk-card"><small>Fee source</small><b>{fee ? 'Account fee data' : 'Futures API'}</b></div></div>; }const pnlDomToPngFile = async (filename) => {
    if (!pnlCardRef.current) throw new Error('PNL card is not ready yet.');
    const dataUrl = await toPng(pnlCardRef.current, { cacheBust: true, pixelRatio: 2, backgroundColor: '#050708' });
    const response = await fetch(dataUrl);
    const blob = await response.blob();
    if (blob.type !== 'image/png') throw new Error('PNG rendering failed.');
...
function PositionHistory({ rows }) { if (!rows.length) return <Empty text="No position history returned." />; return <table><thead><tr><th>Contract</th><th>Side</th><th>Size</th><th>Entry</th><th>Close</th><th>Realized</th><th>Fees</th><th>Created</th></tr></thead><tbody>{rows.map((p, i) => <tr key={p.positionId || i}><td>{displaySymbol(p.symbol)}</td><td>{n(p.positionType) === 1 ? 'Long' : 'Short'}</td><td>{fmt(p.holdVol || p.closeVol)}</td><td>{fmt(p.holdAvgPrice || p.openAvgPrice)}</td><td>{fmt(p.closeAvgPrice)}</td><td>{fmt(p.realised || p.closeProfitLoss)}</td><td>{fmt(p.totalFee || p.fee)}</td><td>{p.createTime ? new Date(n(p.createTime)).toLocaleString() : '—'}</td></tr>)}</tbody></table>; }
function Funding({ rows }) { if (!rows.length) return <Empty text="No funding records returned." />; return <table><thead><tr><th>Contract</th><th>Amount</th><th>Rate</th><th>Position ID</th><th>Time</th></tr></thead><tbody>{rows.map((f, i) => <tr key={f.id || i}><td>{displaySymbol(f.symbol)}</td><td>{fmt(f.amount || f.fundingFee)}</td><td>{pct(f.fundingRate)}</td><td>{f.positionId || '—'}</td><td>{f.createTime ? new Date(n(f.createTime)).toLocaleString() : '—'}</td></tr>)}</tbody></table>; }
function Risk({ risk, fee, positionMode, contract }) { const entries = risk ? Object.entries(risk).flatMap(([symbol, levels]) => Array.isArray(levels) ? levels.map(x => ({ symbol, ...x })) : []) : []; return <div className="risk-grid"><div className="risk-card"><small>Position mode</small><b>{n(positionMode?.positionMode) === 1 ? 'Hedge Mode' : n(positionMode?.positionMode) === 2 ? 'One-way Mode' : '—'}</b></div><div className="risk-card"><small>Contract size</small><b>{fmt(contract?.contractSize)}</b></div><div className="risk-card"><small>Volume unit</small><b>{fmt(contract?.volUnit)}</b></div><div className="risk-card"><small>Price unit</small><b>{fmt(contract?.priceUnit)}</b></div>{entries.slice(0, 12).map((r, i) => <div className="risk-card" key={i}><small>Risk L{r.level} · {n(r.positionType) === 1 ? 'Long' : 'Short'}</small><b>MMR {pct(r.mmr)} · IMR {pct(r.imr)} · Max {fmt(r.maxLeverage, 0)}x</b></div>)}<div className="risk-card"><small>Fee source</small><b>{fee ? 'Account fee data' : 'Futures API'}</b></div></div>; }  useEffect(() => {
    let cancelled = false;
    setPnlShareFile(null);
    if (!pnlSharePosition) { setPnlShareBusy(false); return undefined; }
    setPnlShareBusy(true);
    const timer = window.setTimeout(() => {
      void pnlDomToPngFile(`kitsetups-${String(pnlSharePosition.symbol || 'position').replace(/[^a-z0-9_-]/gi, '')}-pnl.png`)
        .then(file => { if (!cancelled) setPnlShareFile(file); })
        .catch(() => { if (!cancelled) setPnlShareFile(null); })
