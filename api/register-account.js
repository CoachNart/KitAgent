...
async function findExistingCanonicalUser(a,canonical){let pageToken;do{const page=await a.auth().listUsers(1000,pageToken);for(const record of page.users){if(canonicalEmail(record.email)===canonical)return record;}pageToken=page.pageToken}while(pageToken);return null;}
export default async function handler(req,res){
 ...
 try{
  ...
  const canonical=canonicalEmail(email),db=a.firestore(),lockRef=db.collection('accountIdentityLocks').doc(encodeURIComponent(canonical));
  if((await lockRef.get()).exists)return json(res,409,{error:'An account already exists for this email identity. Sign in instead.',code:'ACCOUNT_ALREADY_EXISTS'});
  const legacy=await findExistingCanonicalUser(a,canonical);
  if(legacy)return json(res,409,{error:'An account already exists for this email identity. Sign in instead.',code:'ACCOUNT_ALREADY_EXISTS'});
  const reservation={email,canonicalEmail:canonical,createdAt:admin.firestore.FieldValue.serverTimestamp(),status:'reserved'};
  try{await db.runTransaction(async tx=>{const snap=await tx.get(lockRef);if(snap.exists){const e=new Error('ACCOUNT_ALREADY_EXISTS');e.code=e.message;throw e}tx.create(lockRef,reservation)})}catch(error){if(error?.code==='ACCOUNT_ALREADY_EXISTS')return json(res,409,{error:'An account already exists for this email identity. Sign in instead.',code:error.code});throw error}
  let userRecord;
  try{userRecord=await a.auth().createUser({email,password,emailVerified:false});await lockRef.set({uid:userRecord.uid,status:'active',updatedAt:admin.firestore.FieldValue.serverTimestamp()},{merge:true})}catch(error){await lockRef.delete().catch(()=>{});if(error?.code==='auth/email-already-exists')return json(res,409,{error:'An account already exists with this email. Sign in instead.',code:'ACCOUNT_ALREADY_EXISTS'});throw error}
