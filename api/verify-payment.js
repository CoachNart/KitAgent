...
export default async function handler(req,res){
 ...
 try{
  ...
  const body=typeof req.body==='string'?JSON.parse(req.body||'{}'):(req.body||{}),hash=String(body.transactionHash||'').trim().toLowerCase(),wallet=String(body.walletAddress||'').trim().toLowerCase();
  if(!/^0x[a-f0-9]{64}$/.test(hash))return json(res,400,{error:'Enter a valid transaction hash.'});if(!/^0x[a-f0-9]{40}$/.test(wallet))return json(res,400,{error:'Connect the wallet that made the payment first.'});
  ...
  if(String(tx.chainId||'').toLowerCase()!=='0x38')return json(res,422,{error:'This transaction is not on BNB Smart Chain.'});if(String(tx.from||'').toLowerCase()!==wallet)return json(res,422,{error:'The payment sender does not match the connected wallet.'});
  ...
  const matching=(receipt.logs||[]).find(log=>{const topics=log.topics||[];return String(log.address||'').toLowerCase()===USDT_BSC&&String(topics[0]||'').toLowerCase()===TRANSFER_TOPIC&&normalizeAddress(topics[1])===wallet&&normalizeAddress(topics[2])===PAYMENT_ADDRESS&&parseAmount(log.data)>=PRICE_USDT});
