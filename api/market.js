...
function analyzeCandles(c,forcedBias=null){
  ...
  const priceUnitLabel=(String(symbol||'').includes('/')&& !String(symbol||'').includes('USDT'))?'pips':'price units';
  const pipMultiplier=(String(symbol||'').includes('/')&& !String(symbol||'').includes('USDT'))?(String(symbol||'').includes('JPY')?100:10000):1;
