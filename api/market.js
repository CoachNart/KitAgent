...
function liquidityCandidates(c,bias,entry,atrValue){const s=confirmedSwings(c),src=bias==='LONG'?s.highs:s.lows,out=[];for(const p of src.slice(-30)){const d=bias==='LONG'?(p.price-entry)/entry:(entry-p.price)/entry;if(d<=Math.max(.001,(atrValue/entry)*.3)||d>.12)continue;let touches=1;for(const q of src)if(q.index<p.index&&q.index>=p.index-35&&Math.abs(q.price-p.price)<=Math.max(atrValue*.12,entry*.0004))touches++;out.push({level:p.price,type:touches>=2?(bias==='LONG'?'Equal/clustered highs':'Equal/clustered lows'):(bias==='LONG'?'Internal swing high':'Internal swing low'),touches,distance:d,index:p.index})}return out.sort((a,b)=>b.touches-a.touches||a.distance-b.distance)}
function chooseLiquidityTarget(c,bias,entry,a){const x=liquidityCandidates(c,bias,entry,a)[0];if(!x)return null;const buffer=Math.max(a*.08,entry*.00015),target=bias==='LONG'?x.level-buffer:x.level+buffer;if((bias==='LONG'&&target<=entry)||(bias==='SHORT'&&target>=entry))return null;return{target,type:x.type,liquidityLevel:x.level,touches:x.touches,distancePct:Number((x.distance*100).toFixed(2)),reason:'Target is placed just before confirmed opposing liquidity.'}}
...
function confirmedSwings(c){const highs=[],lows=[];for(let i=3;i<c.length-3;i++){if(pivotHigh(c,i,3,3))highs.push({price:c[i].high,index:i,time:c[i].time});if(pivotLow(c,i,3,3))lows.push({price:c[i].low,index:i,time:c[i].time});}return{highs,lows}}
function marketStructure(c){const s=confirmedSwings(c),h=s.highs.slice(-4),l=s.lows.slice(-4),hh=h.length>=2&&h.at(-1).price>h.at(-2).price,lh=h.length>=2&&h.at(-1).price<h.at(-2).price,hl=l.length>=2&&l.at(-1).price>l.at(-2).price,ll=l.length>=2&&l.at(-1).price<l.at(-2).price;return{trend:hh&&hl?'LONG':lh&&ll?'SHORT':'RANGE',higherHigh:hh,higherLow:hl,lowerHigh:lh,lowerLow:ll,lastHigh:h.at(-1)?.price??null,lastLow:l.at(-1)?.price??null,protectedHigh:h.at(-1)?.price??null,protectedLow:l.at(-1)?.price??null,protectedHighIndex:h.at(-1)?.index??null,protectedLowIndex:l.at(-1)?.index??null,swingsHigh:h,swingsLow:l}}
