...
async function candlesFor(market,symbol,timeframe){const mapped=TIMEFRAME_MAP[timeframe]?.[market==='forex'?'forex':'crypto'];if(!mapped)throw new Error('Unsupported timeframe');if(market==='forex')return fetchYahoo(`${symbol}=X`,mapped);if(market==='metals'){const cfd=CFD_INSTRUMENTS.find(x=>x.symbol===symbol);return fetchYahoo(cfd?.providerSymbol||symbol,mapped);}const clean=symbol.replace(/[^A-Z0-9]/gi,'');if(market==='perpetual'){try{return await fetchBinance(clean,mapped,true)}catch(a){try{return await fetchBybit(clean,timeframe)}catch(b){throw new Error(`Perpetual market data unavailable: ${a.message}; ${b.message}`)}}}return fetchBinance(clean,mapped,false)}
...
function stopForEntry(c,bias,entry,a){
  ...
  const invalidation=bias==='LONG'
    ?(recent.length?Math.max(...recent):Math.min(...c.slice(-20).map(x=>x.low)))
    :(recent.length?Math.min(...recent):Math.max(...c.slice(-20).map(x=>x.high)));
...
function analyzeCandles(c,forcedBias=null,instrumentSymbol=''){
  ...
  const tradeReady=Boolean(trade&&target1!=null&&targetRisk>=2.5&&q.score>=5),status=tradeReady?(targetRisk>=3?'A-GRADE':targetRisk>=2.5?'QUALITY':'ACCEPTABLE'):'WAIT',liquidity=target1?chooseLiquidityTarget(c,bias,entry,a):null;
...
export default async function handler(req,res){if(req.method!=='GET')return json(res,405,{error:'Method not allowed'});try{const decoded=await authenticate(req);await requireActiveAccess(decoded.uid);const market=String(req.query?.market||'forex').toLowerCase(),symbol=String(req.query?.symbol||'').trim().toUpperCase(),timeframe=String(req.query?.timeframe||'1H');if(req.query?.action==='instruments'){
      ...
      if(market==='perpetual'){
        const r=await fetch('https://fapi.binance.com/fapi/v1/exchangeInfo',{headers:{Accept:'application/json'}});
        if(!r.ok)return json(res,502,{error:'Crypto instrument provider unavailable'});
        const body=await r.json();
        const instruments=(body?.symbols||[]).filter(x=>x.status==='TRADING'&&x.quoteAsset==='USDT'&&x.contractType==='PERPETUAL').map(x=>({symbol:x.symbol.replace(/USDT$/,'/USDT'),providerSymbol:x.symbol,name:x.baseAsset+' / USDT',type:'PERPETUAL'}));
