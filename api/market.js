...
function protectedLevels(c){const highs=[],lows=[];for(let i=2;i<c.length-2;i++){if(pivotHigh(c,i))highs.push(c[i].high);if(pivotLow(c,i))lows.push(c[i].low)}return{protectedHigh:highs.at(-1)??null,protectedLow:lows.at(-1)??null,highs:highs.slice(-8),lows:lows.slice(-8)}}
function marketStructure(c){const highs=[],lows=[];for(let i=2;i<c.length-2;i++){if(pivotHigh(c,i))highs.push(c[i].high);if(pivotLow(c,i))lows.push(c[i].low)}const h=highs.slice(-3),l=lows.slice(-3),hh=h.length>1&&h.at(-1)>h.at(-2),lh=h.length>1&&h.at(-1)<h.at(-2),hl=l.length>1&&l.at(-1)>l.at(-2),ll=l.length>1&&l.at(-1)<l.at(-2);return{trend:hh&&hl?'LONG':lh&&ll?'SHORT':'RANGE',higherHigh:hh,higherLow:hl,lowerHigh:lh,lowerLow:ll,protectedHigh:h.at(-1)??null,protectedLow:l.at(-1)??null,lastHigh:h.at(-1)??null,lastLow:l.at(-1)??null}}
