...
import { brokerConfigured, brokerName, brokerCandles, brokerInstrumentSnapshot } from './broker.js';
...
async function candlesFor(market,symbol,timeframe){const mapped=TIMEFRAME_MAP[timeframe]?.[market==='forex'?'forex':market==='metals'?'metals':'crypto'];if(!mapped)throw new Error('Unsupported timeframe');if(market==='forex'||market==='metals'){if(!brokerConfigured()){const e=new Error('Forex / Metal / CFD broker data is not configured. Add OANDA_API_URL, OANDA_API_TOKEN, and OANDA_ACCOUNT_ID before generating setups.');e.code='BROKER_DATA_NOT_CONFIGURED';throw e}const result=await brokerCandles(symbol,timeframe);return result.rows}const clean=symbol.replace(/[^A-Z0-9]/gi,'');if(market==='perpetual'||market==='crypto')return fetchBybit(clean,timeframe);throw new Error('Unsupported market data source')}
...
function strategyPlan(candlesByTf,strategy,instrumentSymbol,executionTimeframe,marketContext='',liveQuote=null){
  ...
  if(bias==='WAIT'){reason='No decisive directional structure is present for the selected strategy and timeframe.';evidence.push('No clear directional structure.');}
    ...
    const level=msnrLevels(current,bias)[0],engulf=candleEngulfing(current,bias),reject=rejectionCandle(current,bias),levelProximity=Boolean(level&&Math.abs(last.close-level.level)<=Math.max(a*1.5,last.close*.002)),levelTouched=Boolean(level&&last.low<=level.level&&last.high>=level.level);
    if(level&&levelProximity&&levelTouched&&(engulf||reject)){const t=evaluateTrade(current,bias,livePrice,a,2.25);if(validTrade(t)){trade=t;entry=livePrice;orderType='MARKET';}else{const z=chooseLimit([{entry:level.level,zone:level}]);if(z){trade=z.trade;entry=z.entry;orderType='LIMIT';}}}
...
export default async function handler(req,res){if(req.method!=='GET')return json(res,405,{error:'Method not allowed'});try{if(String(req.query?.action||'')==='header'){const r=await fetch('https://api.bybit.com/v5/market/tickers?category=linear',{headers:{Accept:'application/json'}});if(!r.ok)return json(res,502,{error:'Bybit ticker provider unavailable'});const body=await r.json();if(body?.retCode!==0||!Array.isArray(body?.result?.list))return json(res,502,{error:body?.retMsg||'Bybit ticker provider unavailable'});const wanted=new Set(['BTCUSDT','ETHUSDT','SOLUSDT','XRPUSDT','BNBUSDT','DOGEUSDT','ADAUSDT','AVAXUSDT','LINKUSDT','SUIUSDT']);const result=body.result.list.filter(x=>wanted.has(x.symbol)).map(x=>({symbol:x.symbol,lastPrice:x.lastPrice,price24hPcnt:x.price24hPcnt}));return json(res,200,{ok:true,result});} const decoded=await authenticate(req);await requireActiveAccess(decoded.uid);const market=String(req.query?.market||'forex').toLowerCase(),symbol=String(req.query?.symbol||'').trim().toUpperCase(),timeframe=String(req.query?.timeframe||'1H');if(req.query?.action==='instruments'){
      if(market==='metals'){
        ...
        if(brokerConfigured()){const brokerRows=await brokerInstrumentSnapshot();const filteredBroker=brokerRows.filter(x=>{const n=String(x.name||'').toUpperCase().replace(/[^A-Z0-9]/g,'');return ['XAUUSD','XAGUSD','US30','US500','NAS100','UK100','GER40','FRA40','JP225','HK50','USOIL','UKOIL'].some(k=>n===k||n.startsWith(k));}).map(x=>({symbol:String(x.name).replace(/[^A-Z0-9]/g,''),providerSymbol:x.name,name:x.displayName||x.name,type:'CFD'})).filter(x=>!query||`${x.symbol} ${x.name}`.toUpperCase().includes(query));return json(res,200,{ok:true,instruments:filteredBroker});} const filtered=query?CFD_INSTRUMENTS.filter(x=>`${x.symbol} ${x.name}`.includes(query)):CFD_INSTRUMENTS;
        return json(res,200,{ok:true,instruments:filtered});
      }
      if(market==='forex'){
        if(brokerConfigured()){const brokerRows=await brokerInstrumentSnapshot();const instruments=brokerRows.filter(x=>/^[A-Z]{3}_[A-Z]{3}$/.test(String(x.name||''))).map(x=>({symbol:String(x.name).replace(/_/g,''),providerSymbol:x.name,name:x.displayName||String(x.name).replace('_',' / '),type:'FOREX'}));return json(res,200,{ok:true,instruments});} const queries=['USD','EUR','GBP','JPY','AUD','NZD','CAD','CHF','SEK','NOK','SGD','HKD','CNH','ZAR','MXN','TRY','PLN','HUF','THB','CZK','ILS','INR','BRL','CLP','COP'];
    ...
    }if(!['forex','crypto','perpetual','metals'].includes(market))return json(res,400,{error:'Unsupported market'});if(!symbol)return json(res,400,{error:'Missing symbol'});if(!allowedIntervals.has(timeframe))return json(res,400,{error:'Unsupported timeframe'});if(market==='forex'&&!/^[A-Z]{6}$/.test(symbol))return json(res,400,{error:'Invalid Forex symbol'});if(market==='metals'&&!CFD_INSTRUMENTS.some(x=>x.symbol===symbol))return json(res,400,{error:'Invalid Metal / CFD symbol'});if((market==='crypto'||market==='perpetual')&&!/^[A-Z0-9]+(?:\/USDT)?$/.test(symbol))return json(res,400,{error:'Invalid crypto symbol'});
  ...
  ],aligned:setup.tradeReady?3:0,totalTimeframes:3,source:market==='forex'||market==='metals'?brokerName()+' broker market data':'Bybit linear perpetuals',generatedAt:new Date().toISOString()})
}catch(e){const code=e?.code||'',status=code==='AUTH_REQUIRED'||code==='AUTH_INVALID'?401:code==='ACCESS_EXPIRED'?403:['BROKER_DATA_NOT_CONFIGURED','BROKER_INSTRUMENT_UNAVAILABLE','BROKER_PRICE_UNAVAILABLE','BROKER_INSUFFICIENT_CANDLES'].includes(code)?503:500;return json(res,status,{ok:false,error:e?.message||'Market analysis failed',code:code||'MARKET_ERROR'})}}