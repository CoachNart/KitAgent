...
function analyzeCandles(c,forcedBias=null){
  ...
  const priceUnitLabel=(String(price||'').includes('/')&& !String(price||'').includes('USDT'))?'pips':'price units';
  const pipMultiplier=(String(price||'').includes('/')&& !String(price||'').includes('USDT'))?(String(price||'').includes('JPY')?100:10000):1;
