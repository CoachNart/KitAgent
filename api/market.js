...
import { biquoteCandles, biquotePrice, biquoteInstrumentSnapshot } from './biquote.js';
...
async function candlesFor(market,symbol,timeframe){const mapped=TIMEFRAME_MAP[timeframe]?.[market==='forex'?'forex':market==='metals'?'metals':'crypto'];if(!mapped)throw new Error('Unsupported timeframe');if(market==='forex'||market==='metals'){if(false){throw new Error('unreachable')}const result=await biquoteCandles(symbol,timeframe);return result.rows}const clean=symbol.replace(/[^A-Z0-9]/gi,'');if(market==='perpetual'||market==='crypto')return fetchBybit(clean,timeframe);throw new Error('Unsupported market data source')}
