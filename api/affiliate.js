...
function makeCode(email){const base=String(email||'KITAGENT').split('@')[0].toUpperCase().replace(/[^A-Z0-9]/g,'').slice(0,8)||'KITAGENT';return `${base}-${crypto.randomBytes(3).toString('hex').toUpperCase()}`}
...
export default async function handler(req,res){
 ...
 try{
  ...
  const db=a.firestore(),userRef=db.collection('users').doc(decoded.uid),userSnap=await userRef.get();if(!userSnap.exists)return json(res,404,{error:'KitAgent account not found.'});
  ...
  if(action==='register'){
   ...
   if(!existing.empty){const data=existing.docs[0].data();return json(res,200,{affiliate:true,referralCode:data.referralCode,commissionRateBps:data.commissionRateBps,status:data.status||'active'})}
   ...
   await ref.set({userId:decoded.uid,email:decoded.email||'',referralCode:code,status:'active',commissionRateBps:rateBps(),totalEarned:0,availableBalance:0,pendingBalance:0,createdAt:now,updatedAt:now});
   return json(res,200,{affiliate:true,referralCode:code,commissionRateBps:rateBps(),status:'active'});
  }
  ...
  return json(res,200,{affiliate:true,referralCode:data.referralCode,commissionRateBps:data.commissionRateBps||rateBps(),status:data.status||'active',totalEarned:Number(data.totalEarned||0),availableBalance:Number(data.availableBalance||0),pendingBalance:Number(data.pendingBalance||0),commissions:rows});
