...
export default async function handler(req,res){try{const op=String(req.query?.op||'');
 ...
 if(op==='margin-mode')return ok(res,{ok:true,result:await privateCall({...credentials,path:'/v5/position/switch-isolated',method:'POST',body:{category:'linear',symbol:b.symbol,tradeMode:b.mode==='isolated'?1:0,buyLeverage:String(b.leverage),sellLeverage:String(b.leverage)}})});
 ...
 return ok(res,{ok:false,error:'Unknown private operation'},400);
}catch(e){return ok(res,{ok:false,error:e?.message||'Bybit request failed'},502)}}
