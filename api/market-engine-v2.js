...
function breakoutRetestEntries(c,bias){
 const a=atr(c,14)||0;
 if(!a||c.length<45)return[];
 const candidates=[];
 for(let i=Math.max(20,c.length-40);i<c.length-2;i++){
  const prior=c.slice(i-20,i),hi=Math.max(...prior.map(x=>x.high)),lo=Math.min(...prior.map(x=>x.low)),x=c[i],range=x.high-x.low,body=Math.abs(x.close-x.open);
  if(!range||body/range<.55)continue;
  const level=bias==='LONG'?hi:lo;
  const broke=bias==='LONG'?x.close>level:x.close<level;
  if(!broke||range<(sma(prior.map(k=>k.high-k.low),20)||range)*1.1)continue;
  let retest=null;
  const zone=Math.max(a*.3,Math.abs(level)*.001);
  for(let j=i+1;j<c.length;j++){
   const k=c[j],touched=bias==='LONG'?k.low<=level+zone:k.high>=level-zone;
   if(!touched)continue;
   const accepted=bias==='LONG'?k.close>level+zone*.15:k.close<level-zone*.15;
   const rejection=bias==='LONG'?(k.close>k.open||k.low<=level-zone*.6):(k.close<k.open||k.high>=level+zone*.6);
   if(accepted&&rejection){
    retest={index:j,level,extreme:bias==='LONG'?Math.min(...c.slice(i+1,j+1).map(q=>q.low)):Math.max(...c.slice(i+1,j+1).map(q=>q.high)),zone};
   }
  }
  if(!retest)continue;
  const entry=c[retest.index].close;
  if(!Number.isFinite(entry)||entry<=0)continue;
  candidates.push({
   entry,
   invalidation:retest.extreme,
   target:null,
   kind:'BREAKOUT + CONFIRMED RETEST',
   pending:false,
   breakoutLevel:level,
   breakoutIndex:i,
...
function strategyEntries(c,bias,strategy,price){
 ...
 if(strategy==='BREAKOUT')return breakoutRetestEntries(c,bias);
