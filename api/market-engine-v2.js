...
function pullbackEntries(c,bias){
 ...
 const impulseStart=bias==='LONG'?lo:hi,impulseEnd=bias==='LONG'?hi:lo;
 const impulse=Math.abs(impulseEnd-impulseStart);
 if(impulse<a*1.5)return out;
 const retrace=bias==='LONG'?hi-impulse*.5:lo+impulse*.5;
 const zone=Math.max(a*.35,Math.abs(retrace)*.001);
 if(!reactionAt(c,bias,retrace,zone))return out;
 const x=c.at(-1),entry=bias==='LONG'?Math.max(retrace,x.close):Math.min(retrace,x.close);
 if(bias==='LONG'&&entry>retrace+zone)return out;
 if(bias==='SHORT'&&entry<retrace-zone)return out;
 out.push({entry,invalidation:bias==='LONG'?lo:hi,target:bias==='LONG'?hi:lo,kind:'CONFIRMED PULLBACK REACTION'});
 return out
}
function breakoutEntries(c,bias){
 const br=rangeBreak(c,bias);if(!br)return[];
 const a=atr(c,14)||0,zone=Math.max(a*.3,Math.abs(br.level)*.001);
 const recent=c.slice(-4),touched=recent.some(x=>x.low<=br.level+zone&&x.high>=br.level-zone);
 const x=c.at(-1),reclaim=bias==='LONG'?x.close>br.level+zone*.1:x.close<br.level-zone*.1;
 if(!touched||!reclaim)return[];
 return[{entry:bias==='LONG'?Math.min(x.close,br.level+zone*.15):Math.max(x.close,br.level-zone*.15),
  invalidation:bias==='LONG'?br.level-zone:br.level+zone,
  target:bias==='LONG'?br.level+a*2.8:br.level-a*2.8,kind:'BREAKOUT RETEST + ACCEPTANCE'}]
}
...
function topDownEntries(c,bias,price){
 ...
 const a=atr(c,14)||0,zone=Math.max(a*.45,Math.abs(price)*.0015);
 if(!reactionAt(c,bias,z.level,zone))return[];
 return[{entry:z.level,invalidation:bias==='LONG'?z.level-zone:z.level+zone,target:targetLevels(c,bias,z.level)[0]?.level,kind:'HTF POI + REACTION'}]
}
function msnrEntries(c,bias,price){
 const z=recentSwing(c,bias,price);if(!z)return[];
 const a=atr(c,14)||0,zone=Math.max(a*.4,Math.abs(price)*.0015);
 if(!reactionAt(c,bias,z.level,zone))return[];
 return[{entry:z.level,invalidation:bias==='LONG'?z.level-zone:z.level+zone,target:targetLevels(c,bias,z.level)[0]?.level,kind:'MSNR LEVEL + REJECTION'}]
}
function crtEntries(c,bias){
 const sw=sweep(c,bias);if(!sw||sw.age>3)return[];
 const a=atr(c,14)||0,zone=Math.max(a*.25,Math.abs(sw.level)*.001);
 if(!reactionAt(c,bias,sw.level,zone))return[];
 const parent=c[Math.max(0,sw.index-1)];
 if(!parent)return[];
