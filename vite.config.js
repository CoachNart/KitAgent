...
const localMarketApi = () => ({
  ...
  configureServer(server) {
    server.middlewares.use(async (req, res, next) => {
      ...
      try { const url=new URL(req.url,'http://localhost'); req.query=Object.fromEntries(url.searchParams.entries()); const handler=req.url.startsWith('/api/perpetual')?perpetualHandler:marketHandler; await handler(req,res); }
      catch(error){res.statusCode=502;res.setHeader('Content-Type','application/json');res.end(JSON.stringify({ok:false,error:error?.message||'Local market API failed'}));}
    });
  },
  configurePreviewServer(server) {
    server.middlewares.use(async (req, res, next) => {
      if (!req.url?.startsWith('/api/market') && !req.url?.startsWith('/api/perpetual')) return next();
      try { const url=new URL(req.url,'http://localhost'); req.query=Object.fromEntries(url.searchParams.entries()); const handler=req.url.startsWith('/api/perpetual')?perpetualHandler:marketHandler; await handler(req,res); }
      catch(error){res.statusCode=502;res.setHeader('Content-Type','application/json');res.end(JSON.stringify({ok:false,error:error?.message||'Preview market API failed'}));}
    });
  },
});

const kitAgentSourceFix = () => ({
  name: 'kitagent-source-fix', enforce: 'pre',
  transform(source,id){
...
export default defineConfig({plugins:[localMarketApi(),kitAgentSourceFix(),react()],server:{port:3000,open:true,strictPort:false},build:{target:'es2020',outDir:'dist',sourcemap:false},preview:{port:4173}});
